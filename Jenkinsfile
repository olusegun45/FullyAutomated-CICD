    ...
    stages {
        stage('Git checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/olusegun45/FullyAutomated-CICD.git'
                sh 'ls'
            }
        }
     ...
     post { 
        always { 
            ...
            slackSend channel: '#webapp-jenkins-cicd-pipeline', color: COLOR_MAP[currentBuild.currentResult], message: "*${currentBuild.currentResult}:* Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} \n More info at: ${env.BUILD_URL}"
        }
    }
